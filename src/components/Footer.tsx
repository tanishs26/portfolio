...
const Footer = () => {
	...
	useEffect(() => {
		animate(
			"span",
			{ opacity: [0, 1], filter: ["blur(10px)", "blur(0px)"], y: [-10, 0],x:[0,-2,2,-2,2,0] },
    ...
    <Container className="p-4 mt-10">
      <div ref={scope} className="w-full flex justify-center ">
        <span className="enq text-secondary  dark:text-secondary text-center font-semibold text-2xl shadow-input/50 px-4 rounded-md py-1">
          {enquiry[count].title}
        </span>
      </div>
